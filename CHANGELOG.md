...
#### v2.0.2 ：2026-9-9
- 优化：移除内部 SubcomposeLayout 预测量机制，指示器改用 onSizeChanged 在真实布局后测量高度——指示器只组合一次（带副作用的第三方自定义指示器不会再执行两份 effect），指示器高度变化不再触发内容重新测量
- 优化：重构 Slingshot 计算为纯函数并返回不可变数据，消除 Header/Footer 复用同一组合时对共享可变状态的反复写入
- 优化：合并 Header/Footer 二级内容的镜像实现与经典指示器重复的文案/时间格式化逻辑（净减约140行，公共API无变化）
- 修复：内部回调状态命名笔误（updateOnLoadMore -> updatedOnLoadMore）
